  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <motion.div
        ...
        className={`fixed inset-0 z-50 bg-fg-1/20 backdrop-blur-sm ${className ?? ""}`}
        {...props}
      />
    )
  }
    ...
    return (
      <DialogPortal>
        ...
        <motion.div
          ...
          animate={{ opacity: 1, scale: 1, transition: { duration: 0.3, ease: easeSwift } }}
          exit={{ opacity: 0, scale: 0.95, transition: { duration: 0.15 } }}
          className={`relative z-50 w-full max-w-lg mx-4 bg-bg-1 rounded-2xl border border-border-1 shadow-card p-6 ${className ?? ""}`}
          {...props}
        >
          {children}
          {showClose && (
            <button
              type="button"
              onClick={() => onOpenChange(false)}
              className="absolute right-4 top-4 rounded-lg p-1.5 text-fg-3 transition-colors hover:bg-fg-1/[.03] hover:text-fg-1"
            >
              <svg
                width="20"
                height="20"
